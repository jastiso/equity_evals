...
ggplot(df, aes(x=med_diff_bio,y=med_diff_wiki)) + 
  geom_hex(bins=70) + scale_fill_continuous(type = "viridis") +
  theme_minimal() 
...
dplyr::n_distinct(fit_df$Evaluatee)
dplyr::n_distinct(fit_df$StudentName)
dplyr::n_distinct(fit_df$FormID)
write.csv2(fit_df, '../data/gender_cleaned.csv')
```
...
contrasts(fit_df$Cid) <- contr.helmert(8)/8
# model
fit = lmer(data=fit_df, med_diff_bio ~ prob.w*Cid + (1|Instructor_ID))
anova(fit)
summary(fit)
```
...
fit = lmer(data=fit_df, med_diff_wiki ~ prob.w*Cid + F_prob.w + (1|Instructor_ID))
anova(fit)
summary(fit)
F_diff_ord = levels(fit_df$Cid)[order(c(fit@beta[3:(3+length(levels(fit_df$Cid))-1)], 0))]
...
p1 <- ggplot(plot_df, aes(x=med_diff_wiki, y=prob.w, color=Cid)) + 
  geom_smooth(alpha=0.0, method='lm', size=.5) + 
  scale_color_brewer(palette='Set1', direction=-1) +
   geom_smooth(data=plot_df, method='lm', size=2, aes(x=med_diff_wiki, y=prob.w), color="black") +  theme_minimal() +
  xlab("Feminine - Masculine Coded Language") + ylab("Femininity of Trainee's First Name")
p1
ggsave("../img/gender.pdf",p1)
```
...
Saving out most biased reviews
```{r}
ex_df <- df %>%
    filter(Question == "Areas for improvement/Additional comments",
      Answer.Text_Detail != "",
...
high_thr = quantile(ex_df$med_diff_wiki, .95)
low_thr = quantile(ex_df$med_diff_wiki, .05)

high_ex <- ex_df %>%
    filter(
      med_man_wiki > high_thr
    )
write.csv(high_ex, './gendered_95.csv')
low_ex <- ex_df %>%
    filter(
      med_man_wiki < low_thr
      )
write.csv(low_ex, './gendered_05.csv')
```
...
```{r}
# save cleaned data
write.csv2(fit_df, '../data/race_eth_cleaned.csv')
```
...
p1 <- ggplot(plot_df, aes(x=log10(text_len), y=asian_mean, color=Cid)) + 
  geom_smooth(alpha=0.0, method='lm', size=.5) + 
  scale_color_brewer(palette='Set1', direction=-1) +
  theme_grey() + geom_smooth(data=plot_df, method='lm', size=2, aes(x=log10(text_len), y=asian_mean), color='black') +  theme_minimal() +
  xlab("Length of Review") + ylab("Likelihood Name is Asian")


#plot_df$Cid <- factor(plot_df$Cid, level=white_len_ord)
p2 <- ggplot(plot_df, aes(x=log10(text_len), y=nh_white_mean, color=Cid)) + 
  geom_smooth(alpha=0.0, method='lm', size=.5) + 
  scale_color_brewer(palette='Set1', direction=-1) +
  theme_minimal()  + geom_smooth(data=plot_df, method='lm', size=2, aes(x=log10(text_len), y=nh_white_mean), color='black') + 
  xlab("Length of Review") + ylab("Likelihood Name is White")

#plot_df$Cid <- factor(plot_df$Cid, level=asian_sent_ord)
p3 <- ggplot(plot_df, aes(x=mean_sent, y=asian_mean, color=Cid)) + 
  geom_smooth(alpha=0.0, method='lm', size=.5) + 
  scale_color_brewer(palette='Set1', direction=-1) +
  theme_grey() + geom_smooth(data=plot_df, method='lm', size=2, aes(x=mean_sent, y=asian_mean), color='black') +  theme_minimal() +
  xlab("Normalized Sentiment of Review") + ylab("Likelihood Name is Asian")


#plot_df$Cid <- factor(plot_df$Cid, level=white_sent_ord)
p4 <- ggplot(plot_df, aes(x=mean_sent, y=nh_white_mean, color=Cid)) + 
  geom_smooth(alpha=0.0, method='lm', size=.5) + 
  scale_color_brewer(palette='Set1', direction=-1) +
  theme_minimal()  + geom_smooth(data=plot_df, method='lm', size=2, aes(x=mean_sent, y=nh_white_mean), color="grey") + 
  xlab("Normalized Sentiment of Review") + ylab("Likelihood Name is White")
...
name_thr = quantile(ex_df$asian_mean, .50)
low_thr = quantile(ex_df$mean_sent, .05)
high_thr = quantile(ex_df$mean_sent, .95)
...
# save for review
write_csv(low_df_asian, '../data/AsianName_05.csv')
high_df_asian <- ex_df %>%
  filter(
    asian_mean > name_thr,
    mean_sent > high_thr
...
# save for review
write_csv(high_df_asian, '../data/AsianName_95.csv')
...
name_thr = quantile(ex_df$nh_white_mean, .50)
high_thr = quantile(ex_df$mean_sent, .95)
low_thr = quantile(ex_df$mean_sent, 0.05)
high_df_white <- ex_df %>%
  filter(
...
# save for review
write_csv(high_df_white, '../data/WhiteName_95.csv')
...
# save for review
write_csv(low_df_white, '../data/WhiteName_05.csv')
...
name_thr = quantile(ex_df$hispanic_mean, .50)
high_thr = quantile(ex_df$mean_sent, .95)
low_thr = quantile(ex_df$mean_sent, .05)
...
# save for review
write_csv(ex_df_hisp, '../data/HispName_95.csv')
...
# save for review
write_csv(ex_df_hisp, '../data/HispName_05.csv')


name_thr = quantile(ex_df$nh_black_mean, .50)
low_thr = quantile(ex_df$mean_sent, .05)
high_thr = quantile(ex_df$mean_sent, .95)
...
# save for review
write_csv(high_df_black, '../data/BlackName_95.csv')
...
# save for review
write_csv(low_df_black, '../data/BlackName_05.csv')
```
...
p1 <- ggplot(plot_df, aes(x=log10(text_len), y=prob.w, color=race)) + 
  geom_smooth(alpha=0.0, method='lm', size=1) + 
  scale_color_brewer(palette='Set2') +
   geom_smooth(data=plot_df, method='lm', size=2, aes(x=log10(text_len), y=prob.w), color='black') +  theme_minimal() +
  xlab("Length of Review") + ylab("Feminiity of Trainee's Name")


p2 <- ggplot(plot_df, aes(x=scale_sent, y=prob.w, color=race)) + 
  geom_smooth(alpha=0.0, method='lm', size=1) + 
  scale_color_brewer(palette='Set2') +
   geom_smooth(data=plot_df, method='lm', size=2, aes(x=scale_sent, y=prob.w), color='black') +  theme_minimal() +
  xlab("Total Sentiment") + ylab("Feminiity of Trainee's Name")

p3 <- ggplot(plot_df, aes(x=mean_sent, y=prob.w, color=race)) + 
  geom_smooth(alpha=0.0, method='lm', size=1) + 
  scale_color_brewer(palette='Set2') +
   geom_smooth(data=plot_df, method='lm', size=2, aes(x=mean_sent, y=prob.w), color='black') +  theme_minimal() +
  xlab("Normalized Sentiment") + ylab("Feminiity of Trainee's Name")
